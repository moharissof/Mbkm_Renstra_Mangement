...
import { VerificationDialog } from "../../_component/VerificationModalWaket";
...
export default function ProgramVerificationPage() {
    ...
    const fetchCompletedPrograms = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/done/waket?bidang_id=${user.jabatan?.bidang_id}&completed=true`
        );
  ...
  const handleVerificationCompleted = async (verified: boolean) => {
    ...
    try {
      const response = await fetch(`/api/done/${selectedProgram.id}/waket`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Verifikasi program yang telah diajukan untuk penyelesaian
            </p>
          </div>
        </div>
