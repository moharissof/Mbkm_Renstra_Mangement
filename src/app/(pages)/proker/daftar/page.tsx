...
import { Clock, Calendar, LockIcon, Filter } from "lucide-react"
...
export default function ApprovedProgramsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "On_Progress":
        return <Badge className="bg-yellow-500 hover:bg-yellow-600">Dikerjakan</Badge>
      case "Done":
      ...
      <div className="space-y-6 p-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-4">
            <Button variant="outline" onClick={() => router.push("/program-kerja")}>
              Lihat Semua Program
            </Button>
          </div>
        </div>
                ...
                >
                  <CardHeader className="p-0">
                    ...
                    >
                      ...
                      <div className="absolute top-4 right-4">{getStatusBadge(program.status)}</div>
                      <div className="absolute top-4 left-4">
                        <Badge className="bg-white/20 backdrop-blur-sm text-white">
                          {program.point_renstra?.bidang?.nama || "Tidak ada bidang"}
                        </Badge>
                      </div>
                      <div className="text-center z-10">
                        <h3 className="text-xl font-bold mb-2 line-clamp-2">{program.nama}</h3>
                        <p className="text-sm opacity-90">
                          {program.point_renstra?.nama || "Tidak ada poin strategis"}
                        </p>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {daysLeft > 0 ? `${daysLeft} hari tersisa` : "Melebihi tenggat waktu"}
                        </span>
                      </div>
                      <Badge variant={daysLeft > 30 ? "outline" : daysLeft > 7 ? "secondary" : "destructive"}>
                        {daysLeft > 30 ? "Jangka Panjang" : daysLeft > 7 ? "Akan Datang" : "Mendesak"}
                      </Badge>
                    </div>
                    ...
                    {program.indikator_proker && program.indikator_proker.length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium mb-2">Indikator Utama:</h4>
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {program.indikator_proker.slice(0, 2).map((indikator: any, idx: number) => (
                            <li key={idx} className="line-clamp-1">
                              • {indikator.nama}: {indikator.target} {indikator.satuan}
                            </li>
                          ))}
                          {program.indikator_proker.length > 2 && (
                            <li>• {program.indikator_proker.length - 2} indikator lainnya</li>
                          )}
                        </ul>
                      </div>
                    )}
                  ...
                  <CardFooter className="p-6 pt-0">
                    ...
                    {program.status === "On_Progress" && (
                      <Button className="w-full" onClick={() => router.push(`/proker/${program.id}`)}>
                        Lanjutkan Program
                      </Button>
                    )}
                    {program.status === "Done" && (
                      <Button
                        className="w-full"
                        variant="secondary"
                        onClick={() => router.push(`/program-kerja/${program.id}`)}
                      >
                        Lihat Detail
                      </Button>
                    )}
