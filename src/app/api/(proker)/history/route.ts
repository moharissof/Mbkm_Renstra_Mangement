...
export async function GET(request: Request) {
  try {
    ...
    const periodeProkerId = searchParams.get("periode_proker_id")
      ? BigInt(searchParams.get("periode_proker_id")!)
      : undefined
    const tahun = searchParams.get("tahun") // Filter by year directly
    ...
    // Add periode proker filter
    if (periodeProkerId) {
      where.periode_proker_id = periodeProkerId
    }

    // Add year filter (alternative to periode_proker_id)
    if (tahun) {
      where.periode_proker = {
        tahun: tahun
      }
    }
    ...
    // Fetch program_kerja items with all relations
    const programKerja = await prisma.program_kerja.findMany({
      ...
      include: {
        ...
        periode_proker: true, // Include periode_proker relation
    ...
    // Get available periode_proker options for filter dropdown
    const availablePeriods = await prisma.periode_proker.findMany({
      where: {
        program_kerja: {
          some: {
            user_id: {
    ...
    const serializedData = serializeBigInt({
      ...
      availablePeriods // Include available periods in response
    })
